...
begin
  ...
  FGameEngine := TGameEngine.Create(
  TILE_SIZE,
  RESOURCES_ROOT,
  ExtractFilePath(ParamStr(0)) + RESOURCES_ROOT + 'levels\'
  );
...
begin
  // Сбрасываем флаги при потере фокуса
  FGameEngine.SetMoveFlags(False, False, False, False);
end;
...
procedure TForm1.FormDeactivate(Sender: TObject);
begin
  // Ничего не делаем (или можно оставить сброс, но теперь он не обязателен)
end;
...
begin
  ...
  // Расчет масштаба для сохранения соотношения сторон
