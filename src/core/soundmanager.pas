...
type
  ...
  private
    ...
    procedure MusicTimerHandler(Sender: TObject);
  public
    constructor Create;
...
begin
  ...
  // Просто проигрываем без repeat - repeat не работает с waveaudio
  ReturnCode := mciSendString('play music', nil, 0, 0);
  ...
  begin
    FMusicPlaying := True;
    FMusicFileName := FileName; // Добавьте это поле в класс
    FMusicLoop := Loop;         // Добавьте это поле в класс

    if Loop then
    begin
      // таймер для музыки (аналогично ветру)
      if not Assigned(FMusicTimer) then
      begin
        FMusicTimer := TTimer.Create(nil);
        FMusicTimer.Interval := 500;
        FMusicTimer.OnTimer := @MusicTimerHandler;
      end;
      FMusicTimer.Enabled := True;
    end;
...
begin
  ...
  begin
    if Assigned(FMusicTimer) then
      FMusicTimer.Enabled := False;
...
// метод для таймера музыки
procedure TSoundManager.MusicTimerHandler(Sender: TObject);
var
  Status: array[0..128] of Char;
begin
  if not FMusicPlaying then Exit;

  // запрашиваем состояние устройства music
  if mciSendString('status music mode', @Status, SizeOf(Status), 0) = 0 then
  begin
    if StrIComp(Status, 'stopped') = 0 then
    begin
      // перезапускаем
      mciSendString('seek music to start', nil, 0, 0);
      mciSendString('play music', nil, 0, 0);
      LogSound('Music restarted by timer');
    end;
  end;
end;
