...
begin
  ...
  // Нормализуем путь
  if Pos('effects\', FileName) = 1 then
    FullPath := ResolvePath(FileName)
  else
    FullPath := ResolvePath('effects\' + FileName);

  LogSound('Attempting to play effect: ' + FullPath);
  ...
  begin
    ...
    if not FileExists(FullPath) then
    begin
      LogError('Alternative path also not found: ' + FullPath);
      Exit;
    end;
  end;
  ...
  begin
    Result := Alias;
    LogSound('Effect played successfully: ' + Alias);
  end
  else
  begin
    FSoundInstances.Remove(Effect);
    Effect.Free;
    LogError('Failed to play effect');
  end;
end;
