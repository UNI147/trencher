...
var
  i: Integer;
  PlayerFootY, PlayerHeadY: Integer;
  SpriteBottomY, SpriteTopY: Integer;
  SpriteHeight: Integer;
begin
  // Получаем координаты игрока
  PlayerHeadY := Round(FPlayer.PixelY); // Верх игрока
  PlayerFootY := Round(FPlayer.PixelY + FMap.TileSize); // Низ игрока
  ...
  begin
    ...
    // Вычисляем верх и низ спрайта (в пикселях)
    ...
    begin
      ...
      // Правило перспективы:
      // Если низ спрайта (его "ноги") выше низа игрока -> спрайт ЗА игроком
      // Если низ спрайта ниже низа игрока -> спрайт ПЕРЕД игроком

      if SpriteBottomY <= PlayerFootY then
      begin
        // Весь спрайт полностью выше или на уровне ног игрока - рисуем ЗА игроком
        SetLength(FBehindSprites, Length(FBehindSprites) + 1);
        FBehindSprites[High(FBehindSprites)] := FSprites[i];
      end
      else
      begin
        // Спрайт перекрывает ноги игрока или полностью ниже - рисуем ПЕРЕД игроком
